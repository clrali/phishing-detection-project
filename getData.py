...
if __name__ == '__main__':
    ...
    loader = URLLoader(maxURLs = 50000)
    cleanTrainingSites, cleanTestSites = loader.getCleanURLs(numURLs = 50000, split = 1.0)
    phishingTrainingSites, phishingTestSites = loader.getPhishingURLs(numURLs = 50000, split = 1.0)

    cleanSiteData = pool_get(cleanTrainingSites)
    phishingSiteData = pool_get(phishingTrainingSites)
    ...
    # save to csv
    legitimate_df.to_csv("datasets/structured_legitimate_data.csv", mode='w', index=False, header=True)
    phishing_df.to_csv("datasets/structured_phishing_data.csv", mode='w', index=False, header=True)
